...
import MurmurFeedItem from '../murmur/MurmurFeedItem';
...
const MyMurmurs = () => {
    ...
    const { data, isLoading, refetch } = useQuery(
        ...
        async () => {
            const response = await axiosPrivet.get('/api/murmurs', {
                params: {
                    page: pageNumber,
    ...
    return (
        <div>
            ...
            {!isLoading &&
                data?.data?.data?.items.map((murmur, index) => (
                    <MurmurFeedItem
                        key={index}
                        murmur={murmur}
                        refetch={refetch}
